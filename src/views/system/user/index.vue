  ...
  methods: {
    ...
    handleResetPwd(row) {
      this.$prompt('请输入"' + row.userName + '"的新密码', "提示", {
        ...
        inputPattern: /^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[!@#$%^&*()_+\-=\[\]{};':"\\|,.<>\/?])[A-Za-z\d!@#$%^&*()_+\-=\[\]{};':"\\|,.<>\/?]{8,20}$/,
        inputErrorMessage: "密码必须8-20位，且包含大小写字母、数字和特殊字符"
      }).then(({ value }) => {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认停用用户编号为"' + userIds + '"的数据项？').then(function() {
