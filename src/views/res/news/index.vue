  ...
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="68px">
      ...
      <el-form-item label="资讯状态" prop="status">
        <el-select v-model="queryParams.status" placeholder="请选择资讯状态" clearable>
          <el-option
    ...
    <el-table v-loading="loading" :data="newsList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="资讯标题"  width="280" align="center" prop="title" />
      ...
      <el-table-column label="资讯状态" align="center" key="status">
            <template slot-scope="scope">
              <el-switch
...
<script>
import { listNews,listApporedNewsIds, getNews, delNews, addNews, updateNews ,changNewsStatus,apporNews,unApporNews} from "@/api/res/news";
...
export default {
  ...
  methods: {
    ...
    handleUpdate(row) {
      ...
      getNews(newsid).then(response => {
        if(response.data.appored!="0"){
          this.$modal.msgSuccess("编号为:" + newsid + "的单据已审核,请撤销审核再修改!");
          return;
        }
     ...
     handleStatusChange(row) {
      ...
      this.$modal.confirm('确认要"' + text + '""' + row.title + '"吗？').then(function() {
        const newsid = row.newsid || this.ids;
        return changNewsStatus(newsid, row.status);
      }).then(() => {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除戒毒资讯编号为"' + newsids + '"的数据项？').then(function() {
    ...
    */
    ...
    async handleDelete(row) {
      try {
        const newsids = row.newsid || this.ids;
        const apporedList = await listApporedNewsIds(newsids)
        if(apporedList.length>0){
          this.$modal.msgSuccess("编号为:" + newsids + "的单据存在已审核单据,请撤销审核再删除!");
          return; 
        }
        else{
          this.$modal.confirm('是否确认删除编号为"' + newsids + '"的数据项？').then(function() {
            return delNews(newsids);
            }).then(() => {
              this.getList();
              this.$modal.msgSuccess("删除成功");
    ...
    handleAppor(row) {
        const newsids = row.newsid || this.ids;
      this.$modal.confirm('是否确认审批发布编号为"' + newsids + '"的数据项？').then(function() {
        //
      }).then(() => {
        this.$refs.publishDialog.Ids=newsids;
    ...
    handlePublishConfirm(result) {
      if (result) {
        if (result.type === "instant") {
          apporNews(2,this.$refs.publishDialog.Ids,"").then(response => {
            this.getList();
            this.$modal.msgSuccess("审批发布成功");
          }).catch(() => {});
        } 
        else if (result.type === "scheduled") {
          apporNews(1,this.$refs.publishDialog.Ids,result.date).then(response =>{
            this.getList();
            this.$modal.msgSuccess("审批发布成功");
          }).catch(() => {});
        }
    ...
    handleUnAppor(row) {
      const newsids = row.newsid || this.ids;
      this.$modal.confirm('是否取消审批发布编号为"' + newsids + '"的数据项？').then(function() {
          return unApporNews(newsids);
      }).then(() => {
          this.getList();
