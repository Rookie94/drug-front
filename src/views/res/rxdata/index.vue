<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="68px">
      ...
      <el-form-item label="处方类型" prop="rxType">
        <el-input
          v-model="queryParams.rxType"
          placeholder="请输入处方类型"
          clearable
          @keyup.enter.native="handleQuery"
        />
    ...
    <el-table v-loading="loading" :data="rxdataList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="处方类型" align="center" prop="rxType" />
      <el-table-column label="处方状态" align="center" key="status">
            <template slot-scope="scope">
              <el-switch
...
<script>
import { listRxdata, getRxdata, delRxdata, addRxdata, updateRxdata,listApporedRxdataIds,changeRxdataStatus,apporRxdata,unApporRxdata } from "@/api/res/rxdata";
...
export default {
  ...
  data() {
    return {
      ...
      rules: {  
        ...
        title: [
          { required: true, message: "处方标题不能为空", trigger: "blur" }
        ],
        content: [
  ...
  methods: {
    ...
    getList() {
      ...
      listRxdata(this.queryParams).then(response => {
        this.rxdataList = response.rows;
    ...
    handleSelectionChange(selection) {
      this.ids = selection.map(item => item.rxId)
    ...
    handleUpdate(row) {
      ...
      getRxdata(rxId).then(response => {
        if(response.data.appored!="0"){
            this.$modal.msgSuccess("编号为:" + rxId + "的单据已审核,请撤销审核再修改!");
            return;
        }      
    ...
    handleStatusChange(row) {
      ...
      this.$modal.confirm('确认要"' + text + '""' + row.title + '"吗？').then(function() {
        const rxId = row.rxId || this.ids;
        return changeRxdataStatus(rxId, row.status);
      }).then(() => {
    ...
    submitForm() {
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.rxId != null) {
            updateRxdata(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addRxdata(this.form).then(response => {
    ...
    /** 删除按钮操作 */    
    // handleDelete(row) {
    //   const rxIds = row.rxId || this.ids;
    //   this.$modal.confirm('是否确认删除戒治处方编号为"' + rxIds + '"的数据项？').then(function() {
    //     return delRxdata(rxIds);
    //   }).then(() => {
    //     this.getList();
    ...
    async handleDelete(row) {
      try {
        const rxIds = row.rxId || this.ids;
        const apporedList = await listApporedRxdataIds(rxIds)
        if(apporedList.length>0){
          this.$modal.msgSuccess("编号为:" + rxIds + "的单据存在已审核单据,请撤销审核再删除!");
          return; 
        }
        else{
          this.$modal.confirm('是否确认删除编号为"' + rxIds + '"的数据项？').then(function() {
            return delRxdata(rxIds);
            }).then(() => {
              this.getList();
              this.$modal.msgSuccess("删除成功");
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('res/rxdata/export', {
        ...this.queryParams
      }, `rxdata_${new Date().getTime()}.xlsx`)
    },
    /** 审批发布操作 */
    handleAppor(row) {
      const rxIds = row.rxId || this.ids;
      this.$modal.confirm('是否确认审批发布编号为"' + rxIds + '"的数据项？').then(function() {
        //
      }).then(() => {
        this.$refs.publishDialog.Ids=rxIds;
    ...
    handlePublishConfirm(result) {
      if (result) {
        if (result.type === "instant") {
          apporRxdata(2,this.$refs.publishDialog.Ids,"").then(response => {
            this.getList();
            this.$modal.msgSuccess("审批发布成功");
          }).catch(() => {});
        } 
        else if (result.type === "scheduled") {
          apporRxdata(1,this.$refs.publishDialog.Ids,result.date).then(response =>{
            this.getList();
            this.$modal.msgSuccess("审批发布成功");
          }).catch(() => {});
        }
    ...
    handleUnAppor(row) {
      const rxIds = row.rxId || this.ids;
      this.$modal.confirm('是否取消审批发布编号为"' + rxIds + '"的数据项？').then(function() {
          return unApporRxdata(rxIds);
      }).then(() => {
          this.getList();
