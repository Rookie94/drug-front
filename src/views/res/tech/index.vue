<template>
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="techList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="技术类型" align="center" prop="techType" v-if="false" />
...
<script>
import { listTech, getTech, delTech, addTech, updateTech,listApporedTechIds,changeTechStatus,apporTech,unApporTech  } from "@/api/res/tech";
import PublishDialog from '@/components/PublishDialog'

export default {
  name: "Tech",
  dicts: ['sys_normal_disable','sys_appor_status'],
  components: {
    PublishDialog
  },
  data() {
  ...
  methods: {
    ...
    handleUpdate(row) {
      ...
      getTech(techId).then(response => {
        if(response.data.appored!="0"){
            this.$modal.msgSuccess("编号为:" + techId + "的单据已审核,请撤销审核再修改!");
            return;
        }  
    ...
    handleStatusChange(row) {
      ...
      this.$modal.confirm('确认要"' + text + '""' + row.title + '"吗？').then(function() {
        const techId = row.techId || this.ids;
        return changeTechStatus(techId, row.status);
      }).then(() => {
    ...
    //   this.$modal.confirm('是否确认删除戒治技术编号为"' + techIds + '"的数据项？').then(function() {
  ...
  async handleDelete(row) {
      try {
        const techIds = row.techId || this.ids;
        const apporedList = await listApporedTechIds(techIds)
        if(apporedList.length>0){
          this.$modal.msgSuccess("编号为:" + techIds + "的单据存在已审核单据,请撤销审核再删除!");
          return; 
        }
        else{
          this.$modal.confirm('是否确认删除编号为"' + techIds + '"的数据项？').then(function() {
            return delTech(techIds);
            }).then(() => {
              this.getList();
              this.$modal.msgSuccess("删除成功");
    ...
    handleAppor(row) {
      const techIds = row.techId || this.ids;
      this.$modal.confirm('是否确认审批发布编号为"' + techIds + '"的数据项？').then(function() {
        //
      }).then(() => {
        this.$refs.publishDialog.Ids=techIds;
    ...
    handlePublishConfirm(result) {
      if (result) {
        if (result.type === "instant") {
          apporTech(2,this.$refs.publishDialog.Ids,"").then(response => {
            this.getList();
            this.$modal.msgSuccess("审批发布成功");
          }).catch(() => {});
        } 
        else if (result.type === "scheduled") {
          apporTech(1,this.$refs.publishDialog.Ids,result.date).then(response =>{
            this.getList();
            this.$modal.msgSuccess("审批发布成功");
          }).catch(() => {});
        }
    ...
    handleUnAppor(row) {
      const techIds = row.techId || this.ids;
      this.$modal.confirm('是否取消审批发布编号为"' + techIds + '"的数据项？').then(function() {
          return unApporTech(techIds);
      }).then(() => {
          this.getList();
