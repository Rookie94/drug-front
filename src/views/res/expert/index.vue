...
<script>
import { listExpert,listApporedExpertIds, getExpert, delExpert, addExpert, updateExpert,changExpertStatus,apporExpert,unApporExpert  } from "@/api/res/expert";
...
export default {
  ...
  methods: {
    ...
    handleUpdate(row) {
      ...
      getExpert(expertid).then(response => {
        if(response.data.appored!="0"){
          this.$modal.msgSuccess("编号为:" + expertid + "的单据已审核,请撤销审核再修改!");
          return;
        }
    ...
    // 机构状态修改
     handleStatusChange(row) {
      ...
      this.$modal.confirm('确认要"' + text + '""' + row.title + '"吗？').then(function() {
        const expertid = row.expertid || this.ids;
        return changExpertStatus(expertid, row.status);
      }).then(() => {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除戒治专家编号为"' + expertids + '"的数据项？').then(function() {
    ...
    */
    ...
    try {
      const expertids = row.expertid || this.ids;
      const apporedList = await listApporedExpertIds(expertids)
      if(apporedList.length>0){
        this.$modal.msgSuccess("编号为:" + expertids + "的单据存在已审核单据,请撤销审核再删除!");
        return; 
      }
      else{
        this.$modal.confirm('是否确认删除编号为"' + expertids + '"的数据项？').then(function() {
          return delExpert(expertids);
          }).then(() => {
            this.getList();
            this.$modal.msgSuccess("删除成功");
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('res/expert/export', {
        ...this.queryParams
    ...
    handleAppor(row) {
      const expertids = row.expertid || this.ids;
      this.$modal.confirm('是否确认审批发布编号为"' + expertids + '"的数据项？').then(function() {
        //
      }).then(() => {
        this.$refs.publishDialog.Ids=expertids;
    ...
    handlePublishConfirm(result) {
      if (result) {
        if (result.type === "instant") {
          apporExpert(2,this.$refs.publishDialog.Ids,"").then(response => {
            this.getList();
            this.$modal.msgSuccess("审批发布成功");
          }).catch(() => {});
        } 
        else if (result.type === "scheduled") {
          apporExpert(1,this.$refs.publishDialog.Ids,result.date).then(response =>{
            this.getList();
            this.$modal.msgSuccess("审批发布成功");
          }).catch(() => {});
        }
    ...
    handleUnAppor(row) {
      const expertids = row.expertid || this.ids;
      this.$modal.confirm('是否取消审批发布编号为"' + expertids + '"的数据项？').then(function() {
        return unApporExpert(expertids);
      }).then(() => {
        this.getList();
