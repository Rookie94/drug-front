<template>
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="resultList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="填报编号" align="center" width="150">
        <template slot-scope="scope">
          <el-link 
            type="primary" 
            :underline="false" 
            @click="handleViewDocs(scope.row)"
            class="result-no-link"
          >
            {{ scope.row.resultNo }}
          </el-link>
        </template>
...
<script>
import { getSurveyInfo, listDocs, getDocs, refreshDocs, delDocs } from "@/api/survey/docresults";
// 引入Dialog组件
import ResultDialog from "@/components/Survey/ResultDialog";
...
export default {
  ...
  data() {
    return {
      ...
      currentSurvey: {},  // 当前查看的量表数据
      currentResult: {}   // 当前查看的填报结果
    };
  },
  created() {
    this.getList();
  ...
  methods: {   
    ...
    handleViewDocs(row) {
      ...
      getDocs(resultId).then(response => {
        ...
        getSurveyInfo(resultData.surveyId).then(response => {
          // 设置当前查看的数据
